 ...
 */
public class Spark33VectorizedParquetRecordReader extends Spark33SpecificParquetRecordReaderBase<Object> {
   ...
   */
  private Spark33ParquetColumnVector[] columnVectors;
  ...
  // For test only.
  public Spark33VectorizedParquetRecordReader(boolean useOffHeap, int capacity) {
    this(
        null,
        "CORRECTED",
        "UTC",
  ...
  private void initBatch(
    ...
    columnVectors = new Spark33ParquetColumnVector[sparkSchema.fields().length];
    for (int i = 0; i < columnVectors.length; i++) {
      columnVectors[i] = new Spark33ParquetColumnVector(parquetColumn.children().apply(i),
  ...
  public boolean nextBatch() throws IOException {
    for (Spark33ParquetColumnVector vector : columnVectors) {
      vector.reset();
    }
    ...
    for (Spark33ParquetColumnVector cv : columnVectors) {
      for (Spark33ParquetColumnVector leafCv : cv.getLeaves()) {
  ...
  private void initColumnReader(PageReadStore pages, Spark33ParquetColumnVector cv) throws IOException {
    if (!missingColumns.contains(cv.getColumn())) {
      if (cv.getColumn().isPrimitive()) {
        ...
        for (Spark33ParquetColumnVector childCv : cv.getChildren()) {
