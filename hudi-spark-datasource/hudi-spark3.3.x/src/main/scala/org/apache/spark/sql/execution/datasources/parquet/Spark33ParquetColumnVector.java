 ...
 */
final class Spark33ParquetColumnVector {
  private final ParquetColumn column;
  private final List<Spark33ParquetColumnVector> children;
  ...
  Spark33ParquetColumnVector(
    ...
    if (isPrimitive) {
      ...
      ValidationUtils.checkArgument(column.children().size() == vector.getNumChildren());
      boolean allChildrenAreMissing = true;

      for (int i = 0; i < column.children().size(); i++) {
        Spark33ParquetColumnVector childCv = new Spark33ParquetColumnVector(column.children().apply(i),
   ...
   */
  List<Spark33ParquetColumnVector> getChildren() {
    return children;
  }
   ...
   */
  List<Spark33ParquetColumnVector> getLeaves() {
    List<Spark33ParquetColumnVector> result = new ArrayList<>();
    getLeavesHelper(this, result);
    return result;
  }

  private static void getLeavesHelper(Spark33ParquetColumnVector vector, List<Spark33ParquetColumnVector> coll) {
    if (vector.isPrimitive) {
      coll.add(vector);
    } else {
      for (Spark33ParquetColumnVector child : vector.children) {
  ...
  void assemble() {
    ...
    if (type instanceof ArrayType || type instanceof MapType) {
      for (Spark33ParquetColumnVector child : children) {
        child.assemble();
      }
      assembleCollection();
